...
class CLIPScore:
    ...
    def __call__(self, predictions, images_root):
        ...
        for img_name, pred_caption in predictions.items():
            ...
            score_individual = self.getCLIPScore(image, pred_caption)
            
            dict_individual[img_name] = score_individual

            print(f'image name : {img_name}, score = {score_individual}')

            total_score += score_individual
        
        json_data = json.dumps(dict_individual, indent=2)

        with open('./individual_clip_score.json', 'w') as file:
